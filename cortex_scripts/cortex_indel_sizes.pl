...
sub print_usage
{
  ...
  print STDERR "Usage: ./cortex_indel_sizes.pl [--limit <k>] [.colour_covgs]\n";
  print STDERR "  Print number of indels of each size\n";
  print STDERR "  Use: ./cortex_indel_sizes.pl --limit 17 calls.colour_covgs\n";
  print STDERR "  to only count 'clean' indels when called with k=17\n";
  exit;
}
...
elsif(@ARGV >= 2)
{
  ...
  {
    print_usage("K limit value invalid '$k_limit'");
  }
}
